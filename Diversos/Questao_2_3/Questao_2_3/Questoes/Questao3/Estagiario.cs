...
namespace Prova_01.Questoes.Questao3
{
  class Estagiario : Colaborador
  {
    ...
    public Estagiario(string funcao, double salario, string registro_estagio, string nome, string endereco, string data_inicio, string tele) : base(nome, endereco, data_inicio, tele)
    {
      this.funcao = funcao;
    ...
    public override void Alocar(string valor, int tipoAlocacao)
    {
      if ((TipoAlocacao)tipoAlocacao == TipoAlocacao.tarefa)
        this.tarefa = valor;
      else
        this.setor = valor;
    ...
    {
      string retorno = string.Format("Nome: {0} - Endereco {1} - Data_inicio: {2} --- funcao: {3} - Salário: {4} - registro_estagio: {5} - tarefa: {6} - setor: {7} - Telefone:{8}"
        , Nome
        , Endereco
        , Data_inicio
         , funcao
